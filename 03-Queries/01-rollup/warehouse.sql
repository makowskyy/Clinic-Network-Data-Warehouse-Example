...
FROM (
    ...
    FROM wizyty fw
    GROUP BY ROLLUP(fw.id_wojewodztwo, fw.id_lekarza, fw.id_leku)
) sub
...
FROM (
    SELECT 
        ...
        COUNT(fw.id_wizyty) AS liczba_wizyt
    FROM wizyty fw
    GROUP BY ROLLUP(fw.id_powiat, fw.id_specjalizacji, fw.id_badania)
) sub
...
FROM (
    SELECT 
        ...
        COUNT(fw.id_wizyty) AS liczba_wizyt
    FROM wizyty fw
    GROUP BY ROLLUP(fw.id_gmina, fw.id_specjalizacji, fw.id_diagnozy)
) sub
